function [ axonTraces, electrodeGroups, ts ] = computeAxonTraces_bkup( fileManagerObj, axonTrackElec, varargin )
...
% [electrodeGroups] = mxw.util.electrodeGroups(axonTrackElec, args.MaxDistClustering);
...
for iFile = 1:nFiles
    ...
    timestamps = cell(size(electrodeGroups));
    ...
    for iChunk = 1:length(chunkSize)
        ...
        for iElecGroup = 1:size(electrodeGroups,1)
            ...
            %spikes = ss_detect(mat2cell(dataCommonElec, size(dataCommonElec,1), ones(1,size(dataCommonElec,2))), spikes);
            ...
            timestamps{iElecGroup} = [timestamps{iElecGroup} spikes.spiketimes+startPoint/20000];
